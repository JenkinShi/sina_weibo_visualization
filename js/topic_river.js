...
function topic_river(){
	...
	function value2len(v){
		return v*(settings.graph_height-settings.text_height)/settings.max_data
	}
	...
	function create_topic_river(){
		tg.svg = d3.select("#"+this.settings.father_id)
					 ...
					 .attr("height",this.settings.graph_height);
		create_cut_off_line();
	...
	function gen_stack_path(){
		...
		tg.topic_pos_last.topic = cal_topic_stock_pos(tg.fix_data.stack[0][0],{}); //话题的y坐标
		for(var i=0; i<2; i++){
			flag = 0;
			for(var j=1; j<tg.fix_data.stack[0].length; j++){
				...
				for(var k=0; k<tg.topic_order.length; k++){
					if(stack_data[tg.topic_order[k]] == undefined){
						...
						tg.topic_pos_now.topic = cal_topic_stock_pos(stack_data,last_stack_data);
			...
			if(flag == 0){
				...
				tg.topic_pos_now.topic = cal_topic_stock_pos(tg.fix_data.stack[i][j-1],{});
				draw_stack_path(tg.topic_pos_last,tg.topic_pos_now,id);
				tg.topic_pos_last = tg.topic_pos_now;
				tg.topic_pos_now = new Object();
			}
	...
	function gen_stack_to_river_path(){
		...
		for(var j=1; j<tg.fix_data.stack[0].length; j++){
			...
			for(var k=0; k<tg.topic_order.length; k++){
				if(stack_data[tg.topic_order[k]] == undefined){
					...
					tg.topic_pos_now.j = j;
					if(j == tg.fix_data.stack[0].length-1){
						tg.topic_pos_now.topic = cal_topic_river_pos(stack_data,last_stack_data,true,0);
						flag = 1;
					}
					else{
						tg.topic_pos_now.topic = cal_topic_river_pos(stack_data,last_stack_data,false,0);
					}
		...
		if(flag == 0){
			...
			tg.topic_pos_now.i = i;
			tg.topic_pos_now.j = tg.fix_data.stack[0].length-1;
			tg.topic_pos_now.topic = cal_topic_river_pos(tg.fix_data.stack[i][j-1],{},true,0);
			draw_river_path(tg.topic_pos_last,tg.topic_pos_now,id);
			tg.topic_pos_last = tg.topic_pos_now;
			tg.topic_pos_now = new Object();
		}
	...
	function cal_topic_archive_pos(data){
		...
		rate = (tg.settings.graph_height - tg.settings.text_height)*0.9/sum;
		length = -(tg.settings.graph_height - tg.settings.text_height)*0.9/2;
		for(var x in data){
			var one_path = [];
			var o = new Object();
	...
	function cal_topic_stock_pos(obj, last_obj){
		var new_topic = [];
		var flag=0;
		var length = 0;
		var topic_pos = [];
	...
	function draw_path(path, name, id){
		var line = d3.svg.line()
			...
			.y(function(t){return (tg.settings.graph_height+tg.settings.text_height)/2-t.y;})
			.interpolate("linear");
		tg.svg.select("#"+id)
			.append("g")
	...
	function draw_river_path(topic_left,topic_right,id){
		...
		while(i<topic_left.topic.length && j<topic_right.topic.length){
			...
			//如果两边命名不相同，则判断哪一边不需要绘制
				...
				//右端跳过一个新话题
				if(j+1<topic_right.topic.length && topic_left.topic[i].name==topic_right.topic[j+1].name){
					offset = gen_path_by_curve(
						topic_left.x,
						topic_left.topic[i].y,
						topic_right.x,
				...
				//左端跳过一个旧话题
				else if(i+1<topic_left.topic.length && topic_left.topic[i+1].name==topic_right.topic[j].name){
					offset = gen_path_by_curve(
						topic_left.x,
	...
	this.add_cut_off_text = function(arr){
		...
		d3.select("#cut_off")
			...
			.enter()
			.append("text")
			.attr("y",settings.text_height-2)
			.attr("fill","black")
			.attr("transform", function(d,i){
				return "rotate(-60 "+that.cut_off_pos[i]+","+settings.text_height+")";
			})
			.attr("x",function(d,i){
